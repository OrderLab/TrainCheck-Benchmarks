...
class GraphAttentionLayer(nn.Module):
    ...
    def __init__(
        ...
        if concat:  # concatenating the attention heads
            ...
            )  # Ensure that out_features is a multiple of n_heads
        ...
        self.softmax = nn.Softmax(
            dim=1
        )  # softmax activation function to the attention coefficients
    ...
    def forward(self, h: torch.Tensor, adj_mat: torch.Tensor):
        ...
        if self.concat:
            h_prime = (
                h_prime.permute(1, 0, 2).contiguous().view(n_nodes, self.out_features)
            )
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--l2", type=float, default=5e-4, help="weight decay (default: 6e-4)"
    )
    parser.add_argument(
        "--dropout-p",
        type=float,
    ...
    if os.path.isfile(os.path.join(path, "cora.content")) and os.path.isfile(
    ...
    gat_net = GAT(
        ...
        num_classes=labels.max().item()
        + 1,  # Number of classes to predict for each node
    ...
    loss_test, acc_test = test(
        gat_net, criterion, (features, adj_mat), labels, idx_test
    )
